...
class MatchupSolver:
    ...
    def find_matchup_solutions(
        ...
        matchup_weights: List[float] = None,
        team_prev_repeats: dict = None,
        matchup_team_repeats: List[dict] = None,
    ) -> List[np.ndarray]:
        """
        ...
        Args:
            ...
            matchup_weights: Optional list of weights for each matchup to maximize.
            team_prev_repeats: Optional dict of {team_id: count} of previous repeats.
            matchup_team_repeats: Optional list of {team_id: count} of repeats in each candidate matchup.
        ...
        if matchup_weights:
            problem += pulp.lpSum(variables[i] * matchup_weights[i] for i in range(len(matchups)))

        if team_prev_repeats is not None and matchup_team_repeats is not None:
            # Enforce fairness: balance total repeats across teams (diff <= 1)
            min_rep = pulp.LpVariable("min_repeats", lowBound=0, cat=pulp.LpInteger)
            max_rep = pulp.LpVariable("max_repeats", lowBound=0, cat=pulp.LpInteger)
            for t in range(1, self.n_teams + 1):
                total_t = team_prev_repeats.get(t, 0) + pulp.lpSum(
                    variables[i] * matchup_team_repeats[i].get(t, 0)
                    for i in range(len(matchups)) if t in matchups[i]
                )
